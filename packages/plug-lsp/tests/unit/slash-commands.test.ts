...
describe('slash commands', () => {
  it('lists configured servers and empty state', async () => {
    expect((await listCommand({ list: () => [] } as never).run('', ctx())).message)
      .toBe('No LSP servers configured.');
    const message = (await listCommand({ list: () => [server('ts')] } as never).run('', ctx())).message;
  ...
  it('starts, stops, and restarts with usage messages', async () => {
    ...
    expect((await startCommand(registry as never).run(' ts ', ctx())).message).toBe('Started LSP server "ts".');
    expect((await stopCommand(registry as never).run('ts', ctx())).message).toBe('Stopped LSP server "ts".');
    expect((await restartCommand(registry as never).run('ts', ctx())).message).toBe('Restarted LSP server "ts".');
  ...
  it('prints buffered diagnostics', async () => {
    ...
    const message = (await diagnosticsCommand({ list: () => [srv] } as never).run('', ctx())).message;
