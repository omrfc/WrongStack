...
describe('scaffoldTool', () => {
  ...
  it('creates npm-package template in dry_run', async () => {
    const ctx = makeCtx();
    const result = await scaffoldTool.execute({ template: 'npm-package', name: 'myproject', dry_run: true }, ctx);
  ...
  it('handles cli-tool template', async () => {
    const ctx = makeCtx();
    const result = await scaffoldTool.execute({ template: 'cli-tool', name: 'mytool', dry_run: true }, ctx);
    expect(result.files_created).toBeGreaterThan(0);
  });

  it('handles react-component template', async () => {
    const ctx = makeCtx();
    const result = await scaffoldTool.execute({ template: 'react-component', name: 'MyComponent', dry_run: true }, ctx);
    expect(result.files_created).toBeGreaterThan(0);
