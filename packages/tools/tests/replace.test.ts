...
describe('replaceTool', () => {
  ...
  it('throws when pattern is missing', async () => {
    const ctx = makeCtx();
    await expect(replaceTool.execute({ files: 'a.txt', pattern: '', replacement: 'x' } as any, ctx)).rejects.toThrow('pattern is required');
  });

  it('throws when replacement is missing', async () => {
    const ctx = makeCtx();
    await expect(replaceTool.execute({ files: 'a.txt', pattern: 'foo', replacement: undefined } as any, ctx)).rejects.toThrow('replacement is required');
  });

  it('throws when files is missing', async () => {
    const ctx = makeCtx();
    await expect(replaceTool.execute({ pattern: 'foo', replacement: 'x' } as any, ctx)).rejects.toThrow('files is required');
  });
  ...
  it('dry_run does not modify files', async () => {
    ...
    const ctx = makeCtx();
    const result = await replaceTool.execute({ pattern: 'world', replacement: 'wstack', files: filePath, dry_run: true }, ctx);
  ...
  it('actually replaces when not dry_run', async () => {
    ...
    await fs.writeFile(filePath, 'hello world', 'utf8');
    const ctx = makeCtx();
    const result = await replaceTool.execute({ pattern: 'world', replacement: 'wstack', files: filePath }, ctx);
    expect(result.files_modified).toBe(1);
  ...
  it('returns empty results when no matches', async () => {
    ...
    await fs.writeFile(filePath, 'hello world', 'utf8');
    const ctx = makeCtx();
    const result = await replaceTool.execute({ pattern: 'nonexistent', replacement: 'x', files: filePath }, ctx);
  ...
  it('handles glob pattern', async () => {
    ...
    const ctx = makeCtx();
    const result = await replaceTool.execute({ pattern: 'foo', replacement: 'baz', files: '*.txt' }, ctx);
  ...
  it('skips binary files', async () => {
    ...
    const ctx = makeCtx();
    const result = await replaceTool.execute({ pattern: 'foo', replacement: 'bar', files: filePath }, ctx);
    expect(result.files_modified).toBe(0);
  });
});