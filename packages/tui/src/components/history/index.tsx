 ...
 */
export function History({ entries, generation, streamingText, toolStream, setSuggestions }: HistoryProps): React.ReactElement {
      ...
      <Static key={generation ?? 0} items={entries}>
        {(entry) => (
          <Box key={entry.id} marginBottom={entry.kind === 'turn-summary' ? 1 : 0}>
            <Entry entry={entry} termWidth={termWidth} setSuggestions={setSuggestions} />
          </Box>
        )}
      </Static>
      {/*
